...
### Not yet migrated
*   `0008_document_embeddings.sql` — written 2026-07-31, **not yet run** against Supabase (Ask CLara Module A — see §4). Enables `pgvector`, adds `document_embeddings` (chunked, RLS on with no policies).
*   `nodes`, `edges` — see prior §1.2 plan in v0.2 history / PRD
...
*   **Phase 3 started (2026-07-31): Ask CLara Module A (embeddings pipeline) built, not yet verified.** See below.
...
1.  **Ask CLara Module B** — retrieval (`match_document_chunks` SECURITY DEFINER function) + the `/ask` page/answer flow, once Module A (embeddings pipeline, built above) is verified against the database.
...
*   [~] Ask CLara Module A — `document_embeddings` (pgvector) + `clara-embed-document` Inngest job — **built 2026-07-31, not yet verified against the database**
*   [ ] Ask CLara Module B — retrieval function (`match_document_chunks`) + `/ask` page
*   [ ] CLara Chatbot — separate surface/pipeline
