...
*   **Transcript quality (2026-08-10; turn formatting 2026-08-20):** Record / Upload audio transcripts are no longer a flat text blob. Default model is **`gpt-4o-transcribe-diarize`** (speakers + segment clocks). Commons Markdown looks like `**Name** · [M:SS]` turns; Session details participants seed `documents.participants` and rename Speaker A/B when possible (solo / one diarized voice → that person; two or more labels stay distinct even if the session lists one host; multi-participant → light LLM map). Whisper fallback (`whisper-1`) keeps clocks every pause / ~20s instead of one block per chunk. Multi-chunk Listens shifts clocks so the full take stays continuous. Set `OPENAI_TRANSCRIPTION_MODEL=whisper-1` for timestamp-only (no speakers). Cost stays ~$0.006/min audio. **Re-transcribe** (per recording, or Admin bulk) re-runs Whisper when original audio is still in `listens-staging` and replaces text + summary.
