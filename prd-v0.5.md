...
1.  **Chat** (`/chat`) — *(Shipped.)* Solo, reflective conversation with CLara; separate pipeline from Ask CLara (no shared prompt, no Commons retrieval). The conversation is ephemeral/in-memory until the participant saves. **"Save full conversation"** writes the whole transcript as one `Type: Reflection` document. **Private by default**, with an explicit **Private / Public** choice at save time. **Per-exchange share** *(2026-08-05)* lets them save one user turn + CLara reply without the whole thread. Saves re-use `createDocument()` + `clara/document.created` so OKF enrichment and Ask CLara embeddings pick them up automatically.
