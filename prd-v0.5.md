...
**Last updated:** 2026-08-16 (edit connections; map click no longer spreads nodes)  
...
*   **Per-element summary** *(2026-08-16):* `documents.summary` holds a short Markdown digest generated automatically after content lands. Detail views (Dashboard Ask pane, Commons popup, document page) default to the **Summary** tab; a second tab shows the original (Transcript / Reflection / Uploaded text). Type `Summary` gathering docs copy `content` into `summary` (no extra LLM). Private Reflect still gets a summary (map extraction stays public-only). Apply **`0028_document_summary.sql`**.
