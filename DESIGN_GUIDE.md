...
- **Map theme wallpapers (Phase 7):** Plant generative topographic field is live on the dashboard map (pan/zoom with nodes). Ocean/Desert palettes stubbed. Themes may be lighter than `--forest-deep` — **node/label/edge contrast is retuned per theme**. Floating chrome stays light `--paper` for readability.  
...
**Knowledge Map:** dark `--forest-deep` canvas by default without wallpaper (`/map` still gets nature sprites). On `/dashboard` the map **fills the viewport under the top nav** (full-bleed) with a **Plant** generative topo wallpaper (smooth gradient + contours in one raster) that pans/zooms with the graph; Ocean/Desert later. **Nodes** are nature sprites (mushrooms / flowers / cacti / leafy) with Fraunces labels; circles are the fallback if a sprite is missing. Active/hover sprites glow; pinned nodes get a contrasting ring. On Plant, labels/edges use dark ink/forest for contrast on the lighter field. Edges are thin **quadratic curves** through sprite centers with a faint animated flow. **Theme wallpapers:** generative topo lives in the same pan/zoom space; optional subtle contour drift; freeze under `prefers-reduced-motion`. **Interactions (Festival harvest pattern):** scroll to zoom, drag background to pan, drag a node to **pin** it (peers reheat around it), double-click to unpin. **Dashboard detail:** selecting a Commons node (or a List card) opens **inside** the floating Ask host — title becomes the element title, summary/transcript fills the enlarged panel, Ask entry stays at the bottom; asking hands off to a scoped Ask thread and clears the selection. Circular **X** closes detail; documents with edit rights show a pencil that opens `DocumentEditor`. On `/map`, detail still overlays the canvas. Respect reduced-motion (freeze the flow).
