...
- **Map theme wallpapers (planned):** Plant / Ocean / Desert generative topographic fields behind the graph (pan/zoom with nodes). Themes may be lighter than `--forest-deep` — **node/label/edge contrast must be retuned per theme**. Floating chrome stays light `--paper` for readability.  
